...
/*
    ...
    A. Można pobrać ShiftDownFunctions.hpp i dodać do projektu / Just download the ShiftDownFunctions.hpp file and add
   to project
    ...
    FetchContent_Declare(ShiftDownFunctions GIT_REPOSITORY https://github.com/DeadDeth/ShiftDownFunctions.git GIT_TAG
   main) FetchContent_MakeAvailable(ShiftDownFunctions) target_link_libraries([program name from add_executable()]
   PRIVATE ShiftDownFunctions)

    To powinno, przynajmniej dla Clion, pobrać wszystko automatycznie z GitHub. / This should automatically download the
   file, at least for Clion, from GitHub.
*/
...
/* Instrukcja Obsługi
    ...
    chce --------------------------------------------------------------------------------------------------------
            ...
            Function nazwa_naszej_funkcji([wartość Tc], [wartość fs], [wartość f], [wartość PHI], [wartość A], [nazwa
    naszej funkcji wzoru]);
    ...
    funkcje --------------------------------------------------------------------------------------------------------
                                                                                   ---- Modulacja ----
                                                          --- Modulowanie funkcji tworzy nowy obiekt i zwraca wskaźnik
    na niego ---
                                             --- dzieki temu nie trzeba się bać że modulacja zmieni wartości oryginalnej
    funkcji przed rysowaniem ---

            Function* [nazwa_funkcji_zmodulowanej] = [nazwa wybranej funkcji modulującej] ([adres pamieci obiektu
    funkcji do modulacji], ["A" funkcji nośnej], ["fs" funkcji nośnej], ["PHI" funkcji nośnej]);
            ...
            Graph [nazwa_obiektu_grafu]([obiekt funkcji do narysowania], [tekst nazwy grafu], [tekst osi X], [tekst osi
    Y]);
    ...
    --------------------------------------------------------------------------------------------------------
                                                        ...
                                                        !!! Pamiętamy o delete dla każdego obiektu stworzonego przez
    funkcje modulujące !!!
    ...
    int main() {
        ...
        ShiftDownFunctions::Graph graf_widma_sinusa(&widmo_sinusa, "Widmo funkcji_sinus", "Częstotliwość [Hz]",
    "Amplituda [A]"); // WIDMO ShiftDownFunctions::Graph graf_sin_modulacja_AM_sin(sin_modulacja_AM_sin, "Modulacja sin,
    AM", "Czas [s]", "Amplituda [A]"); ShiftDownFunctions::Graph graf_sin_modulacja_PM_saw(sin_modulacja_PM_saw,
    "Modulacja saw, PM", "Czas [s]", "Amplituda [A]"); ShiftDownFunctions::Graph
    graf_sin_modulacja_FM_rec(sin_modulacja_FM_rec, "Modulacja rec, FM", "Czas [s]", "Amplituda [A]");
        ShiftDownFunctions::Graph graf_sin_modulacja_FM_tri(sin_modulacja_FM_tri, "Modulacja tri, FM", "Czas [s]",
    "Amplituda [A]");


        //rysowanie w innych kolorach tej samej funkcji
        ShiftDownFunctions::Graph graf_sinus_red_theme(&funkcja_sinus, "funkcja sinus RED", "Czas [s]", "Amplituda [A]",
    Colors::BLOOD_RED, Colors::RED, Colors::PASTEL_RED, Colors::GLASS_RED); ShiftDownFunctions::Graph
    graf_sinus_blue_theme(&funkcja_sinus, "funkcja sinus BLUE", "Czas [s]", "Amplituda [A]", Colors::MIDNIGHT_BLUE,
    Colors::BLUE, Colors::PASTEL_BLUE, Colors::GLASS_BLUE); ShiftDownFunctions::Graph
    graf_sinus_purple_theme(&funkcja_sinus, "funkcja sinus PURPLE", "Czas [s]", "Amplituda [A]", Colors::DARK_PURPLE,
    Colors::PINK, Colors::PASTEL_PURPLE, Colors::PURPLE);
    ...
    --------------------------------------------------------------------------------------------------------
            ...
            Program automatycznie dokona próby zapisu do PNG i na windows i na linux, jeśli natomiast się nie uda to
    zachowa zdjęcia we wskazanym folderze jako bmp, jeśli lokalizacja folderu nie została wskazana ręcznie, program
    zapisze to w miejscu gdzie zapisana jest jego binarka. Mogą występować różne fikuśne błędy i bugi, nie wiem nie
    testowałem każdego scenariusza w razie co można starać się naprawić błąd samemu, sekcji Font lepiej nie ruszac tam
    znajduje się czcionka do którą wkleja się w graf, ma wymiary fizyczne 16 x 16 jakby ktoś chciał podmienic natomiast
    faktyczne litery są 16 x 8, gdzie litera znajduje się z lewej strony tekstury, i taki format musi być zachowany,

            W grafach można modyfikować kolory dla osi, czcionki, tła, wykresu, można podać wartości ARGB jako uint32_t,
    polecam jako hex w formacie 0xAARRGGBB, albo gotowa sekcja kolorów Colors::[nazwa gotowego koloru jaki tam jest]
    ...
    że to funkcjonalność, może kiedyś sam poprawie w wolnym czasie.
            ...
            Windows, a Linux. Domyślnie wszystko jest napisane i przetestowane na Linux Fedora 43, działa bez problemu,
    Na windows też powinno ale nic nie mogę obiecać, niech się cieszą że w ogole mi się chciało o nich pamiętać Zdjęcia
    są w 8k robione, skalowanie z poziomu systemu nie programu ze względu na dosyć leniwe podejście do statycznie
    wyliczanych odległości i zależności między elementami wykresu w celu osiągnięcia najbardziej satysfakcjonujących
    ...
    gdzieś z tyłu głowy.
            ...
            Dla poprawnego działania formatu .png na linux potrzeba zainstalować ImageMagick albo FFmpeg na windows
    jakimś cudem działa powershell script,
...
#pragma region Sekcja czcionki
    ...
    constexpr uint64_t font_Exclam[4] = {0x0000100010001000, 0x1000100000001000, 0x0000000000000000,
                                         0x0000000000000000};
    ...
    constexpr uint64_t font_Dollar[4] = {0x0000100038005000, 0x3800140038001000, 0x0000000000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_Percent[4] = {0x0000000062006400, 0x0800100026004600, 0x0000000000000000,
                                          0x0000000000000000};
    constexpr uint64_t font_Caret[4] = {0x0000100028004400, 0x0000000000000000, 0x0000000000000000, 0x0000000000000000};
    constexpr uint64_t font_Ampers[4] = {0x0000000030004800, 0x3000640048003400, 0x0000000000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_Star[4] = {0x0000000010005400, 0x3800540010000000, 0x0000000000000000, 0x0000000000000000};
    constexpr uint64_t font_ParenL[4] = {0x0000100020004000, 0x4000400040004000, 0x4000200010000000,
                                         0x0000000000000000};
    constexpr uint64_t font_ParenR[4] = {0x0000080004000200, 0x0200020002000200, 0x0200040008000000,
                                         0x0000000000000000};
    ...
    constexpr uint64_t font_Equals[4] = {0x0000000000000000, 0x00003E0000003E00, 0x0000000000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_BracketL[4] = {0x0000300020002000, 0x2000200020002000, 0x2000300000000000,
                                           0x0000000000000000};
    constexpr uint64_t font_BracketR[4] = {0x00000C0004000400, 0x0400040004000400, 0x04000C0000000000,
                                           0x0000000000000000};
    constexpr uint64_t font_BraceL[4] = {0x0000180010001000, 0x2000100010001800, 0x0000000000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_BraceR[4] = {0x0000180008000800, 0x0400080008001800, 0x0000000000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_Pipe[4] = {0x0000100010001000, 0x1000100010001000, 0x1000100000000000, 0x0000000000000000};
    constexpr uint64_t font_Backsl[4] = {0x0000400040002000, 0x2000100010000800, 0x0800000000000000,
                                         0x0000000000000000};
    ...
    constexpr uint64_t font_Apostr[4] = {0x0000180018000000, 0x0000000000000000, 0x0000000000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_Less[4] = {0x0000000000000800, 0x1000200010000800, 0x0000000000000000, 0x0000000000000000};
    constexpr uint64_t font_Greater[4] = {0x0000000000002000, 0x1000080010002000, 0x0000000000000000,
                                          0x0000000000000000};
    ...
    constexpr uint64_t font_Question[4] = {0x0000380044000400, 0x0800100000001000, 0x0000000000000000,
                                           0x0000000000000000};
    ...
    constexpr uint64_t font_Z_kres[4] = {0x080010007C000400, 0x0800100020004000, 0x40007C0000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_Z_krop[4] = {0x100000007C000400, 0x0800100020004000, 0x40007C0000000000,
                                         0x0000000000000000};
    ...
    constexpr uint64_t font_z_kres[4] = {0x0800100000000000, 0x7C00080010002000, 0x40007C0000000000,
                                         0x0000000000000000};
    constexpr uint64_t font_z_krop[4] = {0x1000000000000000, 0x7C00080010002000, 0x40007C0000000000,
                                         0x0000000000000000};
#pragma endregion
} // namespace ShiftDownFonts
#pragma endregion
...
namespace ShiftDownFunctions
{
    ...
    namespace detail
    {
        // saving texture / picture / image, .png or .bmp, png is made via os commands, and is much slower, read
        // instructions
        static void save_texture_to_file(const uint32_t* texture, uint32_t width, uint32_t height,
            ...
            if (FILE* f = fopen(bmp_path.c_str(), "wb")) {
                ...
                if (wants_png) {
#ifdef __linux__
                    std::string linux_cmd = "if command -v magick >/dev/null 2>&1; then magick \"" + bmp_path +
                        "\" \"" + png_path + "\" && rm \"" + bmp_path +
                        "\"; "
                        "elif command -v convert >/dev/null 2>&1; then convert \"" +
                        bmp_path + "\" \"" + png_path + "\" && rm \"" + bmp_path +
                        "\"; "
                        "elif command -v ffmpeg >/dev/null 2>&1; then ffmpeg -y -i \"" +
                        bmp_path + "\" \"" + png_path + "\" && rm \"" + bmp_path + "\"; fi";
                    int unused_linux = std::system(linux_cmd.c_str());
#elif defined(_WIN32) || defined(__NT__)
                    std::string win_cmd =
                        "powershell -Command \"Add-Type -AssemblyName System.Drawing; if (Test-Path '" + bmp_path +
                        "') { $img = [System.Drawing.Image]::FromFile('" + bmp_path + "'); $img.Save('" + png_path +
                        "', [System.Drawing.Imaging.ImageFormat]::Png); $img.Dispose(); Remove-Item '" + bmp_path +
                        "'; }\" >nul 2>&1";
        ...
        static void draw_line(uint32_t* texture, uint32_t texture_width, uint32_t texture_height, uint32_t Ax,
                              uint32_t Ay, uint32_t Bx, uint32_t By, uint32_t color, uint32_t thickness) {
        ...
        static void float_to_engineering_7chars_x(float value, char* buffer) {
            ...
            if (prefix) {
                snprintf(buffer, 8, "%s%s%c", sign == '-' ? "-" : "", temp, prefix);
            }
            else {
                snprintf(buffer, 8, "%s%s", sign == '-' ? "-" : "", temp);
            }
        } // x axis values
        // a very, very slimmed down version of the TextBox class form my graphic engine, made thin enough to work with
        // the library, converts given text to a texture with it, using my own font
        struct TextBox {
                    ...
                    if (current_font_data != nullptr) {
                        ...
                        for (uint64_t n = 0; n < 4; n++) {
                            ...
                            for (uint64_t y = 0; y < 64; y++) {
                                ...
                                case 1:
                                    texture[(current_line + temp_y) * texture_width + (current_letter + temp_x)] =
                                        font_color;
                                    break;
                                default:
                                    break;
        ...
        // for the multigraph legend
        static uint32_t multi_graph_legend(uint32_t* texture, uint32_t y_legend_top_offset, uint32_t padding_top_y,
                                           uint32_t graph_height, uint32_t legend_text_scale, uint32_t texture_width,
                                           uint32_t texture_height, uint32_t divider, uint32_t line_color,
                                           uint32_t background_color, uint32_t font_color, const char* name) {
            ...
            TextBox text(5, 1, background_color);
            uint32_t skala_textu = legend_text_scale / divider;
            uint32_t pozycja_y = (y_legend_top_offset * 16 * skala_textu) + padding_top_y + 12;
            if (pozycja_y >= (graph_height) + padding_top_y + 12) {
                text_chars[0] = '!';
                text_chars[1] = 'R';
                text_chars[2] = 'O';
                text_chars[3] = 'O';
                ...
                text.add_text(text_chars, Colors::MAGENTA);
                return_value = 0;
            }
            else {
                text.add_text(text_chars, line_color);
            }

            uint32_t padding_left = 10;
            for (uint32_t y = 0; y < text.texture_height; y++) {
                uint32_t pozycja_x = 0;

                for (uint32_t x = 0; x < text.texture_width; x++) {
                    for (uint32_t sy = 0; sy < skala_textu; sy++) {
                        for (uint32_t sx = 0; sx < skala_textu; sx++) {
                            texture[((texture_width - (text.texture_width * skala_textu) - padding_left) + sx +
                                     pozycja_x + ((sy + pozycja_y) * texture_width))] =
                                text.texture[x + (y * text.texture_width)];
                        }
                    }
                    pozycja_x += skala_textu;
                }
                pozycja_y += skala_textu;
            }
            return return_value;
        }
...
#pragma region Functions & Graphs
    ...
    // DFT, n^2, very slow for high count of points, included just for flex, or research purposes to compare it with
    ...
    // Faster DFT, n log n instead of n^2, much faster, for 100 samples instead of n^2 = 10 000 operations, we have n
    // log n = about 664 operations, and this scales with n like crazy for DFT
    struct FFT {
    ...
    // this is main class for graph rendering with dft, fft or function objects, read instructions on how to use (it's
    // pretty simple)
    class Graph {
        // index:
        ...
        uint32_t padding_right_x = 284;
        uint32_t padding_top_y = 180;
        uint32_t padding_bot_y = 300;

        // multithreading included in constructor, keep in mind, in order to fully exploit it, capp the max graph
        // rendered in one function to the amount of threads in your cpu in other case it may slow down the code
        // instead, because of the constant context switching by the os, anyway it should work faster with smart usage
    ...
    public:
        Graph(const Function& function_to_render, const char* file_path = nullptr, const char* name_label = " ",
                        ...
                        for (uint32_t sy = 0; sy < skala_textu_name; sy++) {
                            for (uint32_t sx = 0; sx < skala_textu_name; sx++) {
                                texture[(sx + pozycja_x + padding_left_x + (graph_width / 2) +
                                         ((sy + pozycja_y + (16 / divider)) * picture_width[picture_size_index])) -
...
#pragma region axis y label
                ...
                auto* rotated_texture =
                    static_cast<uint32_t*>(_mm_malloc(sizeof(uint32_t) * os_y.texture_width * os_y.texture_height, 32));
                        ...
                        for (uint32_t sy = 0; sy < skala_textu_name; sy++) {
                            for (uint32_t sx = 0; sx < skala_textu_name; sx++) {
                                texture[sx + pozycja_x +
                                        ((sy + pozycja_y + (graph_height / 2) + padding_top_y - center) *
                                         picture_width[picture_size_index])] = rotated_texture[x + y * rotated_width];
                            }
                        }
...
#pragma region values on x
                            ...
                            for (uint32_t sy = 0; sy < skala_textu_value_x; sy++) {
                                for (uint32_t sx = 0; sx < skala_textu_value_x; sx++) {
                                    texture[(sx + pozycja_x + scaled_uint_x[(steps_x * i) - ((steps_x * i) > 0)] +
                                             ((sy + pozycja_y + padding_top_y + graph_height + (16 / divider)) *
                                              picture_width[picture_size_index])) -
                ...
                for (uint32_t y = 0; y < value_x.texture_height; y++) {
                    ...
                    for (uint32_t x = 0; x < value_x.texture_width; x++) {
                        for (uint32_t sy = 0; sy < skala_textu_value_x; sy++) {
                            for (uint32_t sx = 0; sx < skala_textu_value_x; sx++) {
                                texture[(sx + pozycja_x + scaled_uint_x[function_to_render.N - 1] +
                                         ((sy + pozycja_y + padding_top_y + graph_height + (16 / divider)) *
                                          picture_width[picture_size_index])) -
                                        center] = value_x.texture[x + (y * value_x.texture_width)];
                            }
                        ...
                        for (uint32_t x = 0; x < value_y.texture_width; x++) {
                            for (uint32_t sy = 0; sy < skala_textu_value_y; sy++) {
                                for (uint32_t sx = 0; sx < skala_textu_value_y; sx++) {
                                    texture[(padding_left_x + sx + pozycja_x - center - (16 / divider)) +
                                            ((padding_top_y + sy + pozycja_y + (kurwa_zmienna * i) -
                                              ((value_y.texture_height * skala_textu_value_y) / 3)) *
                                             picture_width[picture_size_index])] =
                                        value_y.texture[x + (y * value_y.texture_width)];
                                }
                            }
...
#pragma region segments on x
                ...
                for (uint32_t i = 0; i < sub_segments_count_x; i++) {
                    detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                      (sub_steps_x * i) + padding_left_x,
                                      picture_height[picture_size_index] - padding_bot_y,
                                      (sub_steps_x * i) + padding_left_x, 0 + padding_top_y, sub_segments_color,
                                      sub_segments_thickness / divider);
                }
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                ...
                for (uint32_t i = 0; i < segments_count; i++) {
                    for (uint32_t j = 0; j < sub_segments_count; j++) {
                        detail::draw_line(texture, picture_width[picture_size_index],
                                          picture_height[picture_size_index], padding_left_x,
                                          padding_top_y + (kurwa_zmienna * i) + (kurwa_zmienna_2 * j),
                                          picture_width[picture_size_index] - padding_right_x,
                                          padding_top_y + (kurwa_zmienna * i) + (kurwa_zmienna_2 * j),
                    ...
                    detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                      ...
                                      picture_width[picture_size_index] - padding_right_x,
                                      padding_top_y + (kurwa_zmienna * i), grid_color, grid_thickness / divider);
                }
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
...
#pragma region grid on x
                ...
                for (uint32_t i = 0; i < segments_count; i++) {
                    detail::draw_line(
                        texture, picture_width[picture_size_index], picture_height[picture_size_index],
                        (steps_x * i) + padding_left_x, picture_height[picture_size_index] - padding_bot_y,
                        (steps_x * i) + padding_left_x, 0 + padding_top_y, grid_color, grid_thickness / divider);
                }
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                  scaled_uint_x[function_to_render.N - 1],
                                  picture_height[picture_size_index] - padding_bot_y, graph_width + padding_left_x,
...
#pragma region graph
                // stara oryginalna funkcja, problem dla f > bardzo dużo, oraz Bresenham dostawał zawału i czasy leciały
                // w bardzo dużo for (uint32_t i = 0; i < function_to_render.N - 1; i++) {
                //     detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                //     scaled_uint_x[i], scaled_uint_y[i],scaled_uint_x[i + 1], scaled_uint_y[i + 1], line_color,
                ...
                for (uint64_t i = 0; i < function_to_render.N - 1; i++) {
                    ...
                    i = (scaled_uint_x[i] != scaled_uint_x[i + 1])
                        ? (detail::draw_line(texture, picture_width[picture_size_index],
                                             picture_height[picture_size_index], scaled_uint_x[i], local_max_y,
                                             scaled_uint_x[i], local_min_y, line_color, line_thickness / divider),
                           local_max_y = scaled_uint_y[i + 1], local_min_y = scaled_uint_y[i + 1],
                           detail::draw_line(texture, picture_width[picture_size_index],
                                             picture_height[picture_size_index], scaled_uint_x[i], scaled_uint_y[i],
                                             scaled_uint_x[i + 1], scaled_uint_y[i + 1], line_color,
                                             line_thickness / divider),
                           i)
                        : i;
                }
...
#pragma region save to files and clean up
                detail::save_texture_to_file(texture, picture_width[picture_size_index],
                                             picture_height[picture_size_index], file_path);
...
#pragma region values on x
                ...
                for (uint32_t i = 1; i < segments_count; i++) {
                    ...
                    for (uint32_t y = 0; y < value_x.texture_height; y++) {
                        ...
                        for (uint32_t x = 0; x < value_x.texture_width; x++) {
                            for (uint32_t sy = 0; sy < skala_textu_value_x; sy++) {
                                for (uint32_t sx = 0; sx < skala_textu_value_x; sx++) {
                                    texture[(sx + pozycja_x + (grid_cords * i) + padding_left_x +
                                             ((sy + pozycja_y + padding_top_y + graph_height + (16 / divider)) *
                                              picture_width[picture_size_index])) -
                                            center] = value_x.texture[x + (y * value_x.texture_width)];
                                }
                ...
                for (uint32_t y = 0; y < value_x.texture_height; y++) {
                    ...
                    for (uint32_t x = 0; x < value_x.texture_width; x++) {
                        for (uint32_t sy = 0; sy < skala_textu_value_x; sy++) {
                            for (uint32_t sx = 0; sx < skala_textu_value_x; sx++) {
                                texture[(sx + pozycja_x + graph_width + padding_left_x +
                                         ((sy + pozycja_y + padding_top_y + graph_height + (16 / divider)) *
                                          picture_width[picture_size_index])) -
                                        center] = value_x.texture[x + (y * value_x.texture_width)];
                            }
...
#pragma region grid on x
                for (uint32_t i = 0; i < segments_count; i++) {
                    detail::draw_line(
                        texture, picture_width[picture_size_index], picture_height[picture_size_index],
                        grid_cords * i + padding_left_x, picture_height[picture_size_index] - padding_bot_y,
                        grid_cords * i + padding_left_x, 0 + padding_top_y, grid_color, grid_thickness / divider);
                }
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                  graph_width + padding_left_x, picture_height[picture_size_index] - padding_bot_y,
                                  graph_width + padding_left_x, 0 + padding_top_y, grid_color,
                                  grid_thickness / divider);
#pragma endregion
...
#pragma region wykres
                ...
                //      uint32_t zero_y_pixel = offset_y + padding_top_y;
                //      detail::draw_line(texture, picture_width[picture_size_index],
                //      picture_height[picture_size_index], scaled_uint_x[i], scaled_uint_y[i], scaled_uint_x[i],
                //      zero_y_pixel, line_color, line_thickness / divider);
                //  }
                if (!dB_scale) {
                    uint32_t zero_y_pixel = offset_y + padding_top_y;
                    ...
                    for (uint32_t i = 0; i < K_render; i++) {
                        ...
                        local_peak_y = (scaled_uint_x[i] != scaled_uint_x[i + 1])
                            ? (detail::draw_line(texture, picture_width[picture_size_index],
                                                 picture_height[picture_size_index], scaled_uint_x[i], local_peak_y,
                                                 scaled_uint_x[i], zero_y_pixel, line_color, line_thickness / divider),
                               local_peak_y = zero_y_pixel)
                            : local_peak_y;
                    }
                    detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                      scaled_uint_x[K_render - 1], scaled_uint_y[K_render - 1],
                                      scaled_uint_x[K_render - 1], zero_y_pixel, line_color, line_thickness / divider);
                }
                if (dB_scale) {

                    // drawing from -inf to first point
                    detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                      padding_left_x, padding_top_y + graph_height, padding_left_x, scaled_uint_y[1],
                    ...
                    detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                      scaled_uint_x[K_render - 1], local_max_y, scaled_uint_x[K_render - 1],
                                      local_min_y, line_color, line_thickness / divider);
                }
#pragma endregion

#pragma region save to file and cleaning
                detail::save_texture_to_file(texture, picture_width[picture_size_index],
                                             picture_height[picture_size_index], file_path);
...
#pragma region save to file and clean up
                detail::save_texture_to_file(texture, picture_width[picture_size_index],
                                             picture_height[picture_size_index], file_path);
...
#pragma endregion
    ...
    // Ability to draw, more functions on one canvas, you can keep adding them, and it will draw over the main function
    // scaled graph
    class MultiGraph {
        // index:
        // 0 = 8k default, created and tested with it in mind
        ...
        // legend variables
        uint32_t y_legend_top_offset = 0;
        uint32_t legend_text_scale = 6;
        uint32_t bg = 0;
        uint32_t font = 0;
        uint32_t graph_height_legend = 0;
...
#pragma region grid on y
            ...
            for (uint32_t i = 0; i < segments_count; i++) {
                for (uint32_t j = 0; j < sub_segments_count; j++) {
                    detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                      padding_left_x, padding_top_y + (kurwa_zmienna * i) + (kurwa_zmienna_2 * j),
                                      picture_width[picture_size_index] - padding_right_x,
                                      padding_top_y + (kurwa_zmienna * i) + (kurwa_zmienna_2 * j), sub_segments_color,
                                      sub_segments_thickness / divider);
                }
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                  padding_left_x, padding_top_y + (kurwa_zmienna * i),
...
#pragma region grid on x
            ...
            for (uint32_t i = 0; i < segments_count; i++) {
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                  (steps_x * i) + padding_left_x, picture_height[picture_size_index] - padding_bot_y,
                                  (steps_x * i) + padding_left_x, 0 + padding_top_y, grid_color,
                                  grid_thickness / divider);
            }
            detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                              scaled_uint_x[function_to_render.N - 1],
...
#pragma region wykres
            ...
            for (uint64_t i = 0; i < function_to_render.N - 1; i++) {
                ...
                i = (scaled_uint_x[i] != scaled_uint_x[i + 1])
                    ? (detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                         scaled_uint_x[i], local_max_y, scaled_uint_x[i], local_min_y, line_color,
                                         line_thickness / divider),
                       local_max_y = scaled_uint_y[i + 1], local_min_y = scaled_uint_y[i + 1],
                       detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
...
#pragma region cleaning up and legend
            ...
            y_legend_top_offset += detail::multi_graph_legend(
                ...
                picture_width[picture_size_index], picture_height[picture_size_index], divider, line_color,
                background_color, font_color, name_label);
                        ...
                        for (uint32_t sy = 0; sy < skala_textu_value_x; sy++) {
                            for (uint32_t sx = 0; sx < skala_textu_value_x; sx++) {
                                texture[(sx + pozycja_x + scaled_uint_x[step_x * i] +
                                         ((sy + pozycja_y + padding_top_y + graph_height + (64 / divider)) *
                                          picture_width[picture_size_index])) -
                                        center] = value_x.texture[x + (y * value_x.texture_width)];
                            }
                        }
...
#pragma region grid on x
            for (uint32_t i = 0; i < segments_count; i++) {
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                  ...
                                  scaled_uint_x[step_x * i], 0 + padding_top_y, grid_color, 16 / divider);
            }
            detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
...
#pragma region grid on y
            for (uint32_t i = 0; i < segments_count; i++) {
                detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                                  padding_left_x, padding_top_y + (kurwa_zmienna * i),
                                  picture_width[picture_size_index] - padding_right_x,
                                  padding_top_y + (kurwa_zmienna * i), grid_color, 16 / divider);
            }
            detail::draw_line(texture, picture_width[picture_size_index], picture_height[picture_size_index],
                              padding_left_x, padding_top_y + (graph_height),
                              picture_width[picture_size_index] - padding_right_x, padding_top_y + (graph_height),
...
#pragma endregion
        ...
        void add(const Function& function_to_render, const char* name,
                 uint32_t line_color = ShiftDownFunctionsColorThemes::global_theme_Line_color) {
            // wykres
            int* scaled_x = static_cast<int*>(_mm_malloc(sizeof(int) * function_to_render.N, 32));
            int* scaled_y = static_cast<int*>(_mm_malloc(sizeof(int) * function_to_render.N, 32));
            ...
            uint64_t local_min_y = scaled_uint_y[0];
            for (uint64_t i = 0; i < function_to_render.N - 1; i++) {
                if (scaled_uint_x[i] > picture_width[picture_size_index] - padding_right_x ||
                    scaled_uint_x[i] < padding_left_x)
                    continue;
                if (scaled_uint_y[i] > picture_height[picture_size_index] - padding_bot_y ||
                    scaled_uint_y[i] < padding_top_y)
                    continue;
                local_max_y = scaled_uint_y[i] < local_max_y ? scaled_uint_y[i] : local_max_y;
                local_min_y = scaled_uint_y[i] >= local_min_y ? scaled_uint_y[i] : local_min_y;
                i = (scaled_uint_x[i] != scaled_uint_x[i + 1])
            ...
            y_legend_top_offset +=
                detail::multi_graph_legend(texture, y_legend_top_offset, padding_top_y, graph_height_legend,
                                           legend_text_scale, picture_width[picture_size_index],
                                           picture_height[picture_size_index], divider, line_color, bg, font, name);
        ...
        // When all the functions are added just use it to push the texture on to the disk
...
#pragma region Modulations
    ...
    Modulacje zaimplementowane przeze mnie to:
        ...
        - trójkątna (TRI) i piłokształtna (SAW) -> działa na zasadzie gdzie częstotliwość wchodzi powoli do góry, a
    potem natychmiast spada do wartości początkowej, stosowane w CRT, radarach FMCW

    W skrócie, mała funkcja pakowana w pociąg, wysyłana w świat, odbierana dekodowana i mamy informacje w niej zawarte
                ...
                for (uint64_t k = 0; k < coded_bits[0]; k++) {
                    for (uint64_t d = 1; d <= n; d++) {
                        coded_bits[offset + (1 << k)] = ((d & (1 << k)) != 0)
                            ? (coded_bits[offset + (1 << k)] ^ coded_bits[offset + d])
                            : coded_bits[offset + (1 << k)];
                    }
                }
            }
        }
        ...
        HammingDecoder(uint8_t* bits_to_decode, uint64_t m = 4) {
            ...
            for (uint64_t j = 0; j < frame_count; j++) {
                ...
                for (uint64_t i = 1; i <= n; i++) {
                    ...
                    decoded_bits[decoded_bits_index_global] = (i & (1 << parity_bits_count)) != 0
                        ? (parity_bits_count++, decoded_bits[decoded_bits_index_global])
                        : (decoded_bits_index_local++,
                           decoded_bits[decoded_bits_index_global] |
                               (bits_to_decode[offset + i] << (8 - decoded_bits_index_local)));
                    decoded_bits_index_local =
                        decoded_bits_index_local == 8 ? (decoded_bits_index_global++, 0) : decoded_bits_index_local;
                }
